...
function learndash_add_login_field_top( $content = '' ) {
	...
	if ( ( ! empty( $course_id ) ) && ( in_array( learndash_get_setting( $course_id, 'course_price_type' ), array( 'free' ) ) ) && ( apply_filters( 'learndash_login_form_include_course', true, $course_id ) ) ) {
...
/**
 * Called only durring password set to filter the login_url.
 *
 * @since 3.1.2
 * @param URL     $login_url Current login_url.
...
function learndash_login_form_lostpassword() {
	if ( ( isset( $_REQUEST['redirect_to'] ) ) && ( ! empty( $_REQUEST['redirect_to'] ) ) ) {
		...
		if ( strpos( $redirect_to, 'ld-resetpw=true' ) !== false ) {
			if ( isset( $_POST['user_login'] ) && is_string( $_POST['user_login'] ) ) {
				...
				if ( ( $user ) && ( is_a( $user, 'WP_User' ) ) ) {
					...
					/**
					 * Store the redirect URL in user meta. This will be retreived in
